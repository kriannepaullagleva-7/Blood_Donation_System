<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Blood Record</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('bloods.update', $blood) }}" class="space-y-5">
                        @csrf @method('PUT')

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Donor Name</label>
                            <input type="text" name="donor_name" value="{{ old('donor_name', $blood->donor_name) }}" required
                        ...
                        <div>
                            ...
                            <select name="blood_type"
                                ...
                                @foreach(['A+','A-','B+','B-','AB+','AB-','O+','O-'] as $t)
                                    <option value="{{ $t }}" {{ old('blood_type', $blood->blood_type) == $t ? 'selected' : '' }}>{{ $t }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="bags" value="{{ old('bags', $blood->bags) }}" min="1" required
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent">
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent">
                                @foreach(['available' => 'Available', 'used' => 'Used', 'expired' => 'Expired'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('status', $blood->status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="donation_date" value="{{ old('donation_date', $blood->donation_date) }}" required
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent">
